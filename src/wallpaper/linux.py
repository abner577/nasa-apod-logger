"""Linux wallpaper command handlers grouped by desktop environment."""
...
def _gnome_picture_options_from_resolution_type(resolution_type: str) -> str:
    ...
    return mapping.get(resolution_type, "zoom")
...
def _set_wallpaper_linux_gnome(local_image_path: Path) -> bool:
    ...
    picture_option = _gnome_picture_options_from_resolution_type(
        os.getenv("RESOLUTION_TYPE", "fit").strip().lower() or "fit",
    )
    commands = [
